...
class PermohonanController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        ->whereHas('jenisLayanan', function($query) {
            $query->where('role_tujuan', 'surat_pengantar');
        })
        ->filter($request->all())
        ...
        $layanans = JenisLayanan::where('is_active', true)->where('role_tujuan', 'surat_pengantar')->get();
        ...
        return view('surat-pengantar.permohonan.index', compact(
            'permohonans',
            'stats',
            'statuses',
            'layanans',
    ...
    private function getStats()
    {
        return [
            'total' => Permohonan::whereHas('jenisLayanan', function($q) {
                $q->where('role_tujuan', 'surat_pengantar');
            })->count(),
    ...
    {
        // Cek apakah permohonan ini untuk surat pengantar
        if ($permohonan->jenisLayanan->role_tujuan !== 'surat_pengantar') {
        ...
        return view('surat-pengantar.permohonan.show', compact('permohonan', 'statuses', 'jenisDokumens'));
    }
    ...
    public function updateStatus(Request $request, Permohonan $permohonan)
    {
        try {
            ...
            RiwayatStatus::create([
                ...
                'changed_by' => auth()->id(),
                'keterangan' => $request->keterangan ?? 'Status diperbarui oleh petugas surat pengantar',
                'changed_at' => now(),
            ]);
            ...
            // ===== LOGGING =====
            $this->logWithSubject(
                'Update status permohonan (Surat Pengantar)',
                $permohonan,
    ...
    public function tambahKomentar(Request $request, Permohonan $permohonan)
    {
        try {
            ...
            // ===== LOGGING =====
            $this->logWithSubject(
                'Menambah komentar (Surat Pengantar)',
                $komentar,
    ...
    public function proses(Request $request, Permohonan $permohonan)
    {
        try {
            ...
            RiwayatStatus::create([
                ...
                'changed_by' => auth()->id(),
                'keterangan' => $request->keterangan ?? 'Permohonan diproses oleh petugas surat pengantar',
                'changed_at' => now(),
            ]);
            ...
            // ===== LOGGING =====
            $this->logWithSubject(
                'Memproses permohonan (Surat Pengantar)',
                $permohonan,
    ...
    {
        try {
            Log::info('=== UPLOAD DOKUMEN SURAT PENGANTAR ===');
            ...
            Log::error('Error upload dokumen surat pengantar: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan: ' . $e->getMessage()
            ], 500);
    ...
    public function deleteDokumen(Permohonan $permohonan, $dokumenId)
    {
        try {
            ...
            // ===== LOGGING =====
            $this->logWithSubject(
                'Menghapus dokumen (Surat Pengantar)',
                $dokumen,
            ...
            Log::error('Error delete dokumen surat pengantar: ' . $e->getMessage());
            return response()->json([
                'success' => false,
                'message' => 'Terjadi kesalahan: ' . $e->getMessage()
            ], 500);
