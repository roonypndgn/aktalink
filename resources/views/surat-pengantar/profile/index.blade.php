{{-- resources/views/surat-pengantar/profile/index.blade.php --}}
            ...
            <form id="profileForm" class="profile-form" enctype="multipart/form-data">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group full-width">
                        ...
                        <input type="text" class="form-input" value="{{ $user->role_label ?? 'Surat Pengantar' }}" disabled style="background: #f0f5f2; cursor: not-allowed;">
    ...
    if (photoInput) {
        photoInput.addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                fetch('{{ route('surat-pengantar.profile.update-photo') }}', {
                    method: 'POST',
                    headers: {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (profileForm) {
        profileForm.addEventListener('submit', function(e) {
            ...
            fetch('{{ route('surat-pengantar.profile.update') }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'X-Requested-With': 'XMLHttpRequest',
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Update avatar di sidebar jika tidak ada foto
                    const avatarText = document.getElementById('avatarText');
                    if (avatarText) {
                        avatarText.textContent = data.data.name.substring(0, 2).toUpperCase();
                    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (passwordForm) {
        passwordForm.addEventListener('submit', function(e) {
            ...
            fetch('{{ route('surat-pengantar.profile.update-password') }}', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'X-Requested-With': 'XMLHttpRequest',
